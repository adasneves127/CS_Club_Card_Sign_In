...
def main():
    ...
    while cardID != '':
        ...
        user = hashlib.sha256("".join([x for x in cardID if x.isdigit()]).encode()).hexdigest()[0:32]
        conn = db_conn.conn("csclub-bridgew.mysql.database.azure.com", user, password)
        # # get current user from database
        cardID = hash_card_id(''.join([x for x in cardID if x.isdigit()]))  # Get the card ID from the user, remove the ? and ; from the start and end.
