...
def print_title(width: int):
    ...
    print_formatted_line(title_text, width - 2)

def print_divide(width: int):
    print('+' + '-' * (width - 2) + '+')
    
def print_header(width: int, user_data: list = None):
    print_divide(width)
    print_title(width)
    print_divide(width)
    if user_data is not None:
        print_formatted_line(f'Welcome, {user_data[1]} {user_data[2]}', width - 2)
        print_formatted_line(f"{datetime.now().strftime('%I:%M:%S %p')}", width - 2)
        print_formatted_line(f'Role: {user_data[4]}', width - 2)
        print_divide(width)
    return 3 if user_data is None else 7


def print_mod_header(width: int, user_data: list=None, results=list):
    print_divide(width)
    print_title(width)
    print_divide(width)
    if user_data is not None:
...
def menu_4(width: int):
    ...
    print(f'{"| Please choose an option:":<{width - 1}}' + '|')
    print('|' + ' ' * (width - 2) + '|')
    print(f'{"| 1. Single Sign In":<{width - 1}}' + '|')
    print(f'{"| 2. Batch Sign In":<{width - 1}}' + '|')
    return 5
...
def print_res_list(results: list, width: int):
    ...
    rows = ["|".join([f'{x:<{widths[i]}}' for i, x in enumerate(y)]) for y in results]
    for row in rows:
...
def draw_window(user: list = None, menu_state: int = 0, results: list|str = None):
    ...
    if results is not None:
        ...
        if type(results) is list or type(results) is tuple:
            border = '+' + '+'.join(['-' * x for x in get_col_widths(results)]) + '+'
